...
`updateCumulativeMeter()` queues its work per `(device instance, capability)` behind a private `cumulativeMeterQueues` map (lazily created, not a field initializer - some tests construct devices via `Object.create(Driver.prototype)` without running the constructor) so two close SSE events for the same meter can never interleave the read-modify-write. The actual read-modify-write lives in `runCumulativeMeterUpdate()`: it persists offset/date/lastTotal as one versioned object (`{v, date, lastTotal, offset}`) under a single `meter_<capability>_state` store key in one `setStoreValue()` call, so a store failure can never leave partial state behind - the next event just falls back to the "first run" recalibration path, which recomputes the offset from the live capability value and is safe to re-run. `dateKey` is always a zero-padded ISO `YYYY-MM-DD` string, so plain string comparison orders it correctly against the stored date: equal means a same-day correction (a lower total is clamped, never written as a decrease), greater means a forward rollover (fold the prior day's final total into the offset once), and lesser means a stale/out-of-order event to reject outright. See `test/cumulative-meter.test.ts` for the serialization, rollover, regression-clamp, out-of-order, and store-failure-recovery coverage.
